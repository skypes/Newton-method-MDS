...
//                                "scale=log", //pow//log
            ...
            */
            // multiple alignment
            /*
            args = new String[]{"msa=E:\\tmp_for_mds\\result\\cluster16S\\rrnDB_mafft_auto_no_null.ma",
                                "mode=multiAlignment",
                                "dist=E:\\tmp_for_mds\\result\\cluster16S",
//                                "color=E:\\tmp_for_mds\\result\\cluster16S\\multiAlign2.color.map",
                                "scale=pow", //pow//log
                                "prefix=16Src"
                               };
            */
...
//                                "scale=log", //pow//log
            ...
            */
            // parsnp
            /*
            args = new String[]{"xmfa=E:\\tmp_for_mds\\result\\example_for_parsnp\\parsnp.xmfa",
                                "mode=parsnp",
...
//                                "scale=log", //pow//log
            ...
            */
            // config file
            /*
            args = new String[]{"E:\\tmp_for_mds\\examples\\example_1\\parameters.txt"};
            */
        }
        ...
        if(para.size() == 1){
            String config = para.keySet().iterator().next();
//            System.out.println(config);
            java.util.ArrayList<String> lists = utils.TextTabFileReader.getLineArrFromFile(config);
...
//            para.putAll( utils.TextTabFileReader.getMapFromTabTextFile(config));
//            java.util.HashMap<String, String> utils.TextTabFileReader.getMapFromTabTextFile(config);
...
//            return;
        ...
        if("multiAlignment".equals(mode)){
            ...
            new utils.AdvancedMDS().createMDSImageByIDTmatrixCsvAndColor(distMatrixMap, color, figure, para.get("scale"));
//            new utils.AdvancedMDS().createMDSLableImageByDistanceMatrixCsvAndColor(distMatrixMap, color, figure, null, true);
            return;
        }
            ...
            String color = para.get("color");
            new utils.AdvancedMDS().createMDSLableImageByDistanceMatrixCsvAndColor(matrixCSVFile, color, figure, para.get("scale"), false);
            return;
        }
            ...
            if(para.containsKey("-label")){
                new utils.AdvancedMDS().createMDSLableImageByDistanceMatrixCsvAndColor(matrix, color, figure, para.get("scale"), false);
                return;
            }
            else{
                new utils.AdvancedMDS().createMDSImageByDistanceMatrixCsvAndColor(matrix, color, figure, para.get("scale"), false);
                return;
            }
        }
